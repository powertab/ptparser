...
public:
    ...
    /// @return The font setting used by tablature numbers on the tablature staff
    FontSetting GetTablatureNumbersFontSetting() const
        {return (m_fontSettingArray[DOCUMENT_FONT_SETTING_TABLATURE_NUMBERS]);}
    /// Gets a reference to the font setting used by tablature numbers on the tablature staff
    /// @return A reference to the font setting used by tablature numbers on the tablature staff
    FontSetting& GetTablatureNumbersFontSettingRef()
        {return (m_fontSettingArray[DOCUMENT_FONT_SETTING_TABLATURE_NUMBERS]);}
    /// Gets a constant reference to the font setting used by tablature numbers on the tablature staff
    /// @return A constant reference to the font setting used by tablature numbers on the tablature staff
    ...
    /// @param tablatureStaffLineSpacing Tablature staff line spacing value to validate
    /// @return True if the tablature staff line spacing is valid, false if not
    static bool IsValidTablatureStaffLineSpacing(wxInt32 tablatureStaffLineSpacing)
        {return ((tablatureStaffLineSpacing >= MIN_TABLATURE_STAFF_LINE_SPACING) && (tablatureStaffLineSpacing <= MAX_TABLATURE_STAFF_LINE_SPACING));}
    ...
    bool SetTablatureStaffLineSpacing(wxUint32 tablatureStaffLineSpacing)
        {wxCHECK(IsValidTablatureStaffLineSpacing(tablatureStaffLineSpacing), false); m_tablatureStaffLineSpacing = tablatureStaffLineSpacing; return (true);}
    ...
    // Error Functions
    void DisplaySerializationError(const wxString& fileName, off_t offset, const wxString& errorMessage);
    void DisplayDeserializationError(const wxString& fileName, off_t offset, const wxString& errorMessage);
};
